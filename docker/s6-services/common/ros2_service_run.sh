...
#   Both SERVICE_NAME and ROS2_COMMAND are required.
...
# The command to execute must be provided via ROS2_COMMAND
if [ -z "${ROS2_COMMAND}" ]; then
    echo "[${SERVICE_NAME}] Error: ROS2_COMMAND environment variable must be set" >&2
    exit 1
fi
ROS2_CMD="${ROS2_COMMAND}"
echo "[${SERVICE_NAME}] Executing command: ${ROS2_CMD}"
